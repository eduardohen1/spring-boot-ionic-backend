package br.com.ehsolucoes.cursomc.resources.exception;
...
public class ResourceExceptionHandler {
    ...
    public ResponseEntity<StandardError> dataInegrity(DataIntegrityException e, HttpServletRequest request){
    ...
    public ResponseEntity<StandardError> validation(MethodArgumentNotValidException e, HttpServletRequest request){
        ...
        return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY).body(err); //erro 422 http
    }

    @ExceptionHandler(AuthorizationException.class)
    public ResponseEntity<StandardError> authorization(AuthorizationException e, HttpServletRequest request){
        StandardError err = new StandardError(System.currentTimeMillis(), HttpStatus.FORBIDDEN.value(), "Aceso negado", e.getMessage(), request.getRequestURI());
    ...
    public ResponseEntity<StandardError> amazonService(AmazonServiceException e, HttpServletRequest request){
        ...
        StandardError err = new StandardError(System.currentTimeMillis(), code.BAD_REQUEST.value(), "Erro Amazon Service", e.getMessage(), request.getRequestURI());
        return ResponseEntity.status(code.BAD_REQUEST).body(err);
    }
